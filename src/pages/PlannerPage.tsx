...
export default function PlannerPage() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        try { json = JSON.parse(raw); }
          ...
          throw new Error(res.status === 404
            ? 'Il servizio di pianificazione non risponde (funzioni /api non trovate). Se sei nell\u2019app installata, assicurati di avere l\u2019ultima versione; nel browser ricarica la pagina.'
            : 'Il servizio AI non è configurato: aggiungi OPENAI_API_KEY su Vercel (vedi README).');
        }
      ...
      // 🛡️ Deduplica di sicurezza: mai due giornate con la stessa data,
      // mai due tappe identiche (stesso titolo e stessa ora) nello stesso giorno.
      const seenDate = new Set<string>();
      const cleanGenerated: PlanDay[] = [];
      for (const d of generated) {
        if (seenDate.has(d.date)) {
          const existing = cleanGenerated.find((x) => x.date === d.date)!;
          for (const st of d.stops) {
            const key = `${st.title}|${st.time ?? ''}`;
            if (!existing.stops.some((e) => `${e.title}|${e.time ?? ''}` === key)) existing.stops.push(st);
          }
        } else {
          seenDate.add(d.date);
          const seenStop = new Set<string>();
          const stops = d.stops.filter((st) => {
            const key = `${st.title}|${st.time ?? ''}`;
            if (seenStop.has(key)) return false;
            seenStop.add(key); return true;
          });
          cleanGenerated.push({ ...d, stops });
        }
      }
      generated.length = 0; generated.push(...cleanGenerated);
      ...
      if (groupFlow && firebaseReady()) {
        ...
        try {
          ...
          // Le tappe vivono nel gruppo: il viaggio parte VUOTO e viene riempito
          // dal sync in tempo reale (evita il raddoppio gruppo + copia locale).
          trip.days = [];
          setNewGroup(g);
          sessionStorage.removeItem('zaino-gflow');
        } catch (e) { setErr('Itinerario creato, ma il gruppo no: ' + String((e as Error).message)); }
      }
      update({ trips: [...data.trips, trip], days: trip.days, activeTripId: trip.id });
      setStep(3);
    } catch (e) {
      setErr(
