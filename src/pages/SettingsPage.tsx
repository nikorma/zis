...
import { APP_VERSION, buildDateLabel } from '../lib/version';
  ...
  return (
    <div className="max-w-xl mx-auto p-4 space-y-4">
      ...
      <section className="card text-center space-y-1">
        <p className="font-display font-bold">🎒 Zaino in Spalla</p>
        <p className="text-sm font-mono">{APP_VERSION}</p>
        <p className="text-xs opacity-60">Creata il {buildDateLabel()}</p>
        <p className="text-xs opacity-70">© {new Date().getFullYear()} nikorma — Orma Studio</p>
      </section>
    </div>
  );
}
