...
export async function addGroupStop(
  ...
  // evita doppioni nel gruppo: stessa data, stesso titolo, stessa ora
  try {
    const existing = await getDocs(collection(d, 'groups', groupId, 'stops'));
    const dup = existing.docs.some((x) => {
      const v = x.data() as { title?: string; date?: string; time?: string };
      return v.title === stop.title && v.date === stop.date && (v.time ?? '') === (stop.time ?? '');
    });
    if (dup) return;
  } catch { /* se il controllo fallisce, procedo comunque */ }  const user = await ensureUser();
...
/** Rimuove dal gruppo le tappe doppie (stesso titolo+data+ora): tiene la prima. Solo per l'owner delle tappe. */
export async function dedupeGroupStops(groupId: string): Promise<number> {
  const d = ensure();
  const user = await ensureUser();
  const snap = await getDocs(collection(d, 'groups', groupId, 'stops'));
  const seen = new Set<string>();
  let removed = 0;
  for (const x of snap.docs) {
    const v = x.data() as { title?: string; date?: string; time?: string; ownerId?: string };
    const k = `${v.title}|${v.date}|${v.time ?? ''}`;
    if (seen.has(k)) {
      if (v.ownerId === user.uid) { await deleteDoc(doc(d, 'groups', groupId, 'stops', x.id)); removed++; }
    } else {
      seen.add(k);
    }
  }
...
export function stopsToDays(groupName: string, stops: GroupStop[], prevDays: PersonalDay[]): PersonalDay[] {
  ...
  const days: PersonalDay[] = [...dates].sort().map((date) => {
    ...
    const fromGroup: PersonalStop[] = (byDate[date] ?? [])
      .filter((g) => {
        const k = `${g.title}|${g.time ?? ''}`;
        if (seenGroup.has(k)) return false;
        seenGroup.add(k); return true;
      })
      .map((g) => {
    ...
    // Tappe aggiunte a mano nel viaggio collegato: solo quelle che NON coincidono
    // (per titolo+ora) con una tappa del gruppo → niente doppioni.
    const groupKeys = new Set(fromGroup.map((s) => `${s.title}|${s.time ?? ''}`));
    const manual = (prevDay?.stops ?? []).filter(
      (st) => !st.id.startsWith('g-') && !groupKeys.has(`${st.title}|${st.time ?? ''}`)
    );
